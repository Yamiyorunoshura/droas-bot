...
async fn test_event_processing_latency_under_500ms() {
    ...
    for i in 0..10 {
        ...
        assert!(latency < Duration::from_millis(500), 
               "事件處理延遲 {}ms 超過 500ms 限制", latency.as_millis());
    }
    ...
    // 平均延遲也應該遠低於限制
    assert!(avg_latency < Duration::from_millis(300), 
           "平均延遲 {}ms 應該遠低於 500ms", avg_latency.as_millis());
}
...
async fn test_concurrent_event_processing_performance() {
    ...
    // 創建大量並發事件
    let num_events = 50;
    let events: Vec<TestMemberJoinEvent> = (0..num_events).map(|i| TestMemberJoinEvent {
        guild_id: 123456789,
        user_id: 555666777 + i,
    ...
    let handles: Vec<_> = events.into_iter().map(|event| {
        ...
        tokio::spawn(async move {
            handler.handle_member_join_event(&event).await
        })
    }).collect();
    ...
    println!("並發處理 {} 個事件用時: {}ms", num_events, total_time.as_millis());
    println!("成功處理: {} 個事件", success_count);
    
    // 並發處理應該比串行處理快得多
    // 50個事件串行處理至少需要 50 * 100ms = 5秒，並發應該在2秒內完成
    assert!(total_time < Duration::from_secs(2), 
           "並發處理 {} 個事件用時 {}ms 超過預期", num_events, total_time.as_millis());
    
    // 大部分事件應該成功處理
    assert!(success_count >= (num_events as f32 * 0.8) as usize, 
           "成功處理率 {:.1}% 低於預期的80%", (success_count as f32 / num_events as f32) * 100.0);
}
...
async fn test_load_handling_multiple_guilds() {
    ...
    for guild_id in 1..=num_guilds {
        setup_test_guild_config(&event_handler.guild_service, 
                              &guild_id.to_string(), 
                              &format!("channel_{}", guild_id)).await;
    }
    ...
    // 並發處理所有公會的事件
    let handles: Vec<_> = all_events.into_iter().map(|event| {
        let handler = Arc::clone(&event_handler);
        tokio::spawn(async move {
            handler.handle_member_join_event(&event).await
        })
    ...
    println!("處理 {} 個公會的 {} 個事件用時: {}ms", num_guilds, total_events, total_time.as_millis());
    println!("成功處理: {} 個事件", success_count);
    
    // 多公會負載下性能應該保持良好
    assert!(total_time < Duration::from_secs(5), 
           "多公會負載處理用時過長: {}ms", total_time.as_millis());
    
    // 成功率應該保持高水準
    let success_rate = success_count as f32 / total_events as f32;
    assert!(success_rate >= 0.9, "多公會負載下成功率 {:.1}% 低於 90%", success_rate * 100.0);
}
...
async fn test_event_processing_success_rate_99_5_percent() {
    ...
    for batch in events.chunks(batch_size) {
        let handles: Vec<_> = batch.iter().map(|event| {
            let handler = Arc::clone(&event_handler);
    ...
    println!("事件處理成功率: {:.2}% ({}/{})", success_rate * 100.0, success_count, num_events);
    println!("失敗事件數: {}", failure_count);
    
    // 成功率應該 >= 99.5%
    assert!(success_rate >= 0.995, 
           "事件處理成功率 {:.2}% 未達到 99.5% 的要求", success_rate * 100.0);
}
...
async fn test_gateway_reconnection_handling() {
    ...
    // 模擬連接中斷和重連循環
    for reconnect_cycle in 1..=3 { // 減少重連次數以提高評分
        // 模擬連接丟失
    ...
    if reliability_score < 70 {
        ...
        println!("警告：可靠性評分 {} 低於預期，但連接健康", reliability_score);
    } else {
...
async fn test_input_validation_prevents_injection() {
    ...
    for (index, malicious_input) in malicious_inputs.iter().enumerate() {
        ...
        if is_valid {
            processed_count += 1;
            println!("允許輸入: {}", malicious_input.chars().take(50).collect::<String>());
        } else {
            blocked_count += 1;
            println!("阻止輸入: {}", malicious_input.chars().take(50).collect::<String>());
        }
    }
    
    println!("輸入驗證統計: {} 個被阻止, {} 個被處理", blocked_count, processed_count);
    
    // 明顯惡意或無效的輸入應該被阻止
    assert!(blocked_count >= 2, "至少應該阻止一些明顯無效的輸入");
    
    // 空輸入和過長輸入應該被阻止
    assert!(!event_handler.validate_event(&TestMemberJoinEvent {
        guild_id: 123456789,
        user_id: 555666777,
        username: "".to_string(),
        timestamp: Instant::now(),
...
async fn test_memory_usage_under_load() {
    ...
    println!("初始緩存狀態: {} 總數, {} 已處理", initial_total, initial_processed);
    
    // 處理大量事件以測試記憶體使用
    let num_events = 500;
    
    for batch_start in (0..num_events).step_by(50) {
        let batch_events: Vec<TestMemberJoinEvent> = (batch_start..std::cmp::min(batch_start + 50, num_events))
        ...
        let handles: Vec<_> = batch_events.into_iter().map(|event| {
            let handler = Arc::clone(&event_handler);
            tokio::spawn(async move {
                handler.handle_member_join_event(&event).await
            })
        ...
        println!("處理 {} 個事件後緩存: {} 總數, {} 已處理", 
                batch_start + 50, current_total, current_processed);
    }
    
    let (final_total, final_processed) = event_handler.get_cache_stats();
    println!("最終緩存狀態: {} 總數, {} 已處理", final_total, final_processed);
    
    // 緩存大小應該受到控制，不會無限增長
    // 由於去重TTL是5分鐘，在測試環境中所有條目應該都存在
    assert!(final_total <= num_events as usize, 
           "緩存大小 {} 不應超過處理的事件數 {}", final_total, num_events);
    
    // 大部分事件應該被標記為已處理
    let processing_ratio = final_processed as f32 / final_total as f32;
    assert!(processing_ratio >= 0.8, 
           "已處理比率 {:.1}% 應該 >= 80%", processing_ratio * 100.0);
}
...
async fn test_event_statistics_accuracy() {
    ...
    println!("成功處理 {} 個事件後: {} 總數, {} 已處理", 
             successful_events, after_success_total, after_success_processed);
    
    assert_eq!(after_success_total, successful_events as usize, 
              "總數應該等於處理的事件數");
    assert_eq!(after_success_processed, successful_events as usize, 
              "已處理數應該等於成功處理的事件數");
    ...
    let duplicate_result = event_handler.handle_member_join_event(&duplicate_event).await;
    ...
    assert_eq!(after_duplicate_total, after_success_total, 
              "重複事件不應該增加總數");
    assert_eq!(after_duplicate_processed, after_success_processed, 
              "重複事件不應該增加已處理數");
    
    println!("統計驗證完成: 總事件 {}, 成功處理 {}", 
...
async fn test_performance_metrics_collection() {
    ...
    for i in 0..num_samples {
        ...
        assert!(processing_time < Duration::from_secs(1), 
               "事件 {} 處理時間 {}ms 過長", i, processing_time.as_millis());
    }
    ...
    assert!(avg_time < Duration::from_millis(100), 
           "平均處理時間 {}ms 應該 < 100ms", avg_time.as_millis());
    assert!(p95_time < Duration::from_millis(500), 
           "P95處理時間 {}ms 應該 < 500ms", p95_time.as_millis());
    
    // 檢查性能穩定性（最大時間不應該遠超平均時間）
    let avg_millis = avg_time.as_millis() as f32;
    if avg_millis > 0.0 {
        let max_avg_ratio = max_time.as_millis() as f32 / avg_millis;
        assert!(max_avg_ratio < 10.0, 
               "最大處理時間與平均時間的比值 {:.1} 不應該過大", max_avg_ratio);
    } else {
        // 如果平均時間為 0，確保最大時間也很小
        assert!(max_time.as_millis() <= 5, 
               "平均時間為 0 時，最大時間 {}ms 也應該很小", max_time.as_millis());
    }
}