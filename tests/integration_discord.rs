...
async fn test_discord_client_integration() {
    // 測試環境設置
    std::env::set_var("DISCORD_BOT_TOKEN", "NDkxNjM4NzExODE0MzY4Mjc3.YH5K_w.test_token_do_not_use_in_production");
    ...
    // 創建 Discord 客戶端
    let client = DiscordClient::new(Arc::clone(&config)).await
        .expect("應該能夠創建 Discord 客戶端");
    
    // 驗證客戶端狀態
    let initial_status = client.get_gateway_status().await;
    assert!(matches!(initial_status, droas_bot::discord::gateway::GatewayStatus::Disconnected));
...
async fn test_rate_limiter_integration() {
    use droas_bot::discord::rate_limit::{RateLimiter, ExponentialBackoffConfig};
    ...
    // 設置速率限制並測試
    limiter.handle_rate_limit_response("test_route", 0.01, false).await; // 10ms 限制
    let wait_time = limiter.wait_if_rate_limited("test_route").await;
