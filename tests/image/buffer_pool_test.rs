use droas_bot::image::{BufferPool, ImageBuffer, WELCOME_IMAGE_WIDTH, WELCOME_IMAGE_HEIGHT};
use tokio_test;
...
async fn test_buffer_pool_performance() {
    ...
    assert!(duration.as_millis() < 1000, "Buffer operations took too long: {:?}", duration);
}