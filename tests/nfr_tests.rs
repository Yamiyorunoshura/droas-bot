...
mod nfr_tests {
    ...
    async fn test_security_no_token_in_debug_output() {
        ...
        assert!(!debug_output.contains("secret_test_token_12345"), 
                "Token should not appear in debug output: {}", debug_output);
        
        // Verify debug output contains [HIDDEN] instead
        assert!(debug_output.contains("[HIDDEN]"), 
                "Debug output should contain [HIDDEN]: {}", debug_output);
    ...
    async fn test_performance_config_load_time() {
        ...
        assert!(duration < Duration::from_millis(100), 
                "Config loading took too long: {:?}", duration);
    ...
    async fn test_reliability_graceful_error_handling() {
        ...
        assert!(config_result.is_err(), "Config loading should fail with missing env vars");
        
        // Error message should be helpful
        let error_msg = config_result.unwrap_err().to_string();
        assert!(error_msg.contains("DISCORD_BOT_TOKEN") || error_msg.contains("required"),
                "Error should mention missing token: {}", error_msg);
    ...
    async fn test_async_architecture_non_blocking() {
        ...
        assert!(duration < Duration::from_millis(80), 
                "Async operations should run concurrently: {:?}", duration);
    }
    ...
    fn test_logging_security() {
        ...
        assert!(!log_message.contains(sensitive_token), 
