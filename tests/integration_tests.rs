...
fn test_config_mod_exists() {
    ...
    assert!(
        config_mod_path.exists(),
        "src/config/mod.rs file should exist"
    );

    let config_secrets_path = Path::new("src/config/secrets.rs");
    assert!(
        config_secrets_path.exists(),
        "src/config/secrets.rs file should exist"
    );
}
...
fn test_handlers_mod_exists() {
    ...
    assert!(
        handlers_mod_path.exists(),
        "src/handlers/mod.rs file should exist"
    );
}
...
fn test_required_dependencies_in_cargo_toml() {
    ...
    assert!(
        cargo_toml_content.contains("serenity"),
        "serenity dependency should be present"
    );
    assert!(
        cargo_toml_content.contains("tokio"),
        "tokio dependency should be present"
    );
    assert!(
        cargo_toml_content.contains("sqlx"),
        "sqlx dependency should be present"
    );
}
