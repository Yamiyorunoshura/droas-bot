...
async fn test_guild_member_add_event_reception() {
    ...
    // 驗證 Gateway 連接狀態仍然健康
    assert!(gateway_manager.is_connection_healthy(), "Gateway 連接應該保持健康");
}
...
async fn test_event_validation_invalid_event() {
    ...
    // 驗證無效事件被拒絕
    assert!(!event_handler.validate_event(&invalid_event_1), "Guild ID 為 0 的事件應該被拒絕");
    assert!(!event_handler.validate_event(&invalid_event_2), "User ID 為 0 的事件應該被拒絕");
    assert!(!event_handler.validate_event(&invalid_event_3), "用戶名為空的事件應該被拒絕");
}
...
async fn test_event_deduplication() {
    ...
    match result2 {
        Ok(EventResult::Skipped(reason)) => {
            assert!(reason.contains("duplicate") || reason.contains("重複"), 
                   "跳過原因應該提到重複事件");
        },
        Err(e) => {
            assert!(e.to_string().contains("duplicate") || e.to_string().contains("重複"), 
                   "錯誤訊息應該提到重複事件");
        },
        _ => panic!("重複事件應該被去重處理"),
    }
...
async fn test_event_processing_pipeline() {
    ...
    // 創建多個測試事件
    let events: Vec<TestMemberJoinEvent> = (0..5).map(|i| TestMemberJoinEvent {
        guild_id: 123456789,
        user_id: 555666777 + i,
    ...
    for result in results {
        let event_result = result.expect("任務不應該 panic")
            .expect("事件處理應該成功");
