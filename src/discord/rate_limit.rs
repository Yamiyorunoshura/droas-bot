...
impl RateLimiter {
    ...
    pub async fn wait_if_rate_limited(&self, route: &str) -> Duration {
        ...
        if let Some(wait_time) = self.check_global_rate_limit().await {
            ...
            if let Some(ref monitor) = self.monitor {
                monitor
                    .record_rate_limit_event(route, wait_time.as_millis() as u64, true)
                    .await;
            }
        }
        ...
        if let Some(wait_time) = self.check_route_rate_limit(route).await {
            ...
            if let Some(ref monitor) = self.monitor {
                if !had_global_limit {
                    monitor
                        .record_rate_limit_event(route, wait_time.as_millis() as u64, false)
                        .await;
                }
            }
        }
    ...
    /// * `global` - 是否為全域限制
    pub async fn handle_rate_limit_response(&self, route: &str, retry_after: f64, global: bool) {
            ...
            match operation().await {
                ...
                Err(error) => {
                    ...
                    if retry_state.attempt >= self.backoff_config.max_retries {
                        debug!(
                            "路由 {} 達到最大重試次數 {}，停止重試",
                            route, self.backoff_config.max_retries
                        );
                        self.retry_states.write().await.remove(route);
                        return Err(error);
                    }
                    ...
                    retry_state.next_delay_ms = std::cmp::min(
                        (retry_state.next_delay_ms as f64 * self.backoff_config.backoff_multiplier)
                            as u64,
...
mod tests {
    ...
    async fn test_rate_limit_info_update() {
        ...
        // 更新速率限制信息
        limiter
            .update_rate_limit_info("test_route", 100, 50, 30.0)
            .await;
    ...
    async fn test_cleanup_expired_limits() {
        ...
        // 添加短期限制 (0.05 秒)
        limiter
            .handle_rate_limit_response("test_route", 0.05, false)
            .await;
    ...
    async fn test_exponential_backoff_max_retries() {
        ...
        // 模擬始終失敗的操作
        let result = limiter
            .retry_with_exponential_backoff("failing_route", || async {
    ...
    async fn test_retry_state_reset() {
        ...
        let _result = limiter
            .retry_with_exponential_backoff("reset_test", move || {
