...
impl SetBackgroundHandler {
    async fn handle_impl(&self, ctx: CommandContext) -> CommandResult<()> {
        ...
        // 更新公會配置
        self.update_guild_config(&ctx, guild_id.0 as i64, &asset_id)
            .await?;
...
impl SetBackgroundHandler {
        ...
        if let Some(attachment_option) = ctx.get_option("attachment") {
            if let CommandDataOptionValue::Attachment(attachment_id) = &attachment_option.resolved {
                let attachment = ctx
                    ...
                    .get(attachment_id)
                    .ok_or_else(|| {
                        CommandError::InvalidArguments("找不到指定的附件".to_string())
                    })?;
    ...
    async fn update_guild_config(
        ...
        match ctx.config_service.update_config(&config).await {
            ...
            Err(e) => {
                error!("配置服務錯誤：{}", e);
                Err(CommandError::Internal(crate::error::DroasError::database(
                    format!("配置服務錯誤：{}", e),
                )))
            }
        }
...
mod tests {
    ...
    fn create_test_handler() -> (SetBackgroundHandler, TempDir) {
        ...
        let http_service =
            Arc::new(HttpService::with_default_config().expect("創建 HTTP 服務失敗"));
        let handler =
            SetBackgroundHandler::new(temp_dir.path().to_string_lossy().to_string(), http_service);
        (handler, temp_dir)
    }
