        ...
        self.interaction
            .create_interaction_response(&self.discord_ctx.http, |response| {
                response
                    .kind(InteractionResponseType::ChannelMessageWithSource)
                    .interaction_response_data(|message| {
                        message.content(format!("❌ {}", error)).ephemeral(true)
                    })
            })
            .await
            .map_err(CommandError::from)
...
impl CommandFramework {
    ...
    pub async fn execute_command(
        ...
        match result {
            Ok(Ok(())) => {
                info!("命令執行成功 - 命令: '{}', 用戶: {}", command_name, user_id);
                Ok(())
            }
            Ok(Err(e)) => {
                error!(
            ...
            Err(_) => {
                error!("命令執行超時 - 命令: '{}', 用戶: {}", command_name, user_id);
    ...
    async fn check_permissions(
        ...
        let guild_id = ctx.guild_id().ok_or_else(|| {
            CommandError::InsufficientPermissions("此命令只能在伺服器中使用".to_string())
        })?;
        ...
        let member = ctx
            ...
            .ok_or_else(|| {
                CommandError::InsufficientPermissions("無法獲取用戶權限信息".to_string())
            })?;

        let permissions = member
            .permissions(&ctx.discord_ctx.cache)
...
mod tests {
    ...
    async fn test_command_framework_creation() {
        ...
        assert_eq!(
            framework_with_timeout.command_timeout,
