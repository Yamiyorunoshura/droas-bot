...
impl CommandHandler for ConfigHandler {
    async fn handle(&self, ctx: CommandContext) -> CommandResult<()> {
        ...
        match subcommand.resolved {
            CommandDataOptionValue::String(ref action) => {
                match action.as_str() {
        ...
        command
            ...
            .create_option(|option| {
                option
                    ...
                    .required(true)
                    .add_string_choice("查看配置", "view")
                    .add_string_choice("重置配置", "reset")
            })
    }
}
...
impl ConfigHandler {
    ...
    async fn format_config_display(&self, config: &GuildConfig) -> CommandResult<String> {
        ...
        display.push_str(&format!("📅 **創建時間**：<t:{}:F>\n", config.created_at.timestamp()));
        display.push_str(&format!("🔄 **最後更新**：<t:{}:R>\n\n", config.updated_at.timestamp()));
...
mod tests {
    ...
    fn test_default_implementation() {
        ...
        assert_eq!(handler1.required_permissions(), handler2.required_permissions());
    }
}