...
impl PreviewHandler {
    async fn handle_impl(&self, ctx: CommandContext) -> CommandResult<()> {
        ...
        let preview_image = self
            .generate_preview_image(username, &avatar_url, background_path.as_deref())
            .await?;
...
impl PreviewHandler {
        ...
        match ctx.config_service.get_config(guild_id).await {
            Ok(Some(config)) => {
                if let Some(background_ref) = config.background_ref {
                    let background_path =
                        format!("{}/backgrounds/{}.png", self.assets_dir, background_ref);
                    ...
                    if tokio::fs::metadata(&background_path).await.is_ok() {
                        ...
                        // 嘗試 .jpg 檔案
                        let jpg_path =
                            format!("{}/backgrounds/{}.jpg", self.assets_dir, background_ref);
    ...
    async fn generate_preview_image(
        ...
        let preview_image = self
            .compose_welcome_image(username, &avatar_data, background_data.as_deref())
            .await?;
...
mod tests {
    ...
    fn test_get_user_avatar_url() {
        ...
        let avatar_url = runtime
            .block_on(handler.get_user_avatar_url(&user_with_avatar))
            .unwrap();
        ...
        let default_avatar_url = runtime
            .block_on(handler.get_user_avatar_url(&user_without_avatar))
            .unwrap();
