...
impl EventHandler {
    ...
    /// 3. 委託給歡迎處理器
    /// 4. 性能監控
    pub async fn handle_member_join_event(&self, event: &TestMemberJoinEvent) -> Result<EventResult> {
        let start_time = Instant::now();
        debug!("開始處理成員加入事件: guild_id={}, user_id={}", event.guild_id, event.user_id);
    ...
    async fn check_duplication(&self, event: &TestMemberJoinEvent) -> Option<String> {
        ...
        if let Some(entry) = cache.get(&cache_key) {
            if entry.processed && entry.timestamp.elapsed() < self.dedup_ttl {
                return Some(format!("重複事件: guild_id={}, user_id={}", event.guild_id, event.user_id));
            }
        }

        // 記錄事件開始處理（防止並發重複）
        cache.insert(cache_key, DeduplicationEntry {
    ...
    async fn process_member_join_internal(&self, event: &TestMemberJoinEvent) -> Result<()> {
        debug!("處理成員加入事件: guild_id={}, user_id={}", event.guild_id, event.user_id);

        // 查詢公會配置
        let guild_config = self.guild_service
            .get_guild_config(&event.guild_id.to_string())
            .await?;

        if let Some(config) = guild_config {
            debug!("找到公會配置: welcome_channel_id={}", config.welcome_channel_id);
            ...
            info!("歡迎訊息處理完成: guild_id={}, user_id={}", event.guild_id, event.user_id);
        } else {
            warn!(
                "未找到公會配置，跳過歡迎訊息: guild_id={}",
                event.guild_id
            );
        }
    ...
    pub fn get_cache_stats(&self) -> (usize, usize) {
        ...
        let processed_entries = cache.values()
