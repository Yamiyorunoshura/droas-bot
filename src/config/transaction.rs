        ...
        self.execute_write_transaction(guild_ids, move |repo| {
            ...
            Box::pin(async move {
                for config in configs {
                    repo.upsert_config(&config).await.with_context(|| {
                        format!("批量更新配置失敗: guild_id={}", config.guild_id)
                    })?;
                }
                Ok(())
            })
...
mod tests {
    ...
    async fn test_read_transaction() {
        ...
        let result = manager
            .execute_read_transaction(vec![12345], |repo| {
                Box::pin(async move { repo.get_config(12345).await })
            })
            .await
            .expect("只讀事務執行失敗");

        match result {
            TransactionResult::Committed(config_result) => match config_result {
    ...
    async fn test_write_transaction() {
        ...
        let result = manager
            .execute_write_transaction(vec![54321], |repo| {
                Box::pin(async move { repo.upsert_config(&config).await })
            })
            .await
            .expect("寫事務執行失敗");
    ...
    async fn test_transaction_stats() {
        ...
        // 執行一些操作以創建鎖
        let _result = manager
            .execute_read_transaction(vec![1, 2, 3], |_repo| Box::pin(async move { Ok(()) }))
            .await;
