...
use crate::config::{
    cache::{CacheStats, GuildConfigCache},
    models::{BackgroundAsset, GuildConfig},
...
impl GuildConfigService {
    ...
    pub async fn new(
        ...
        repository
            .migrate()
            .await
            .context("配置服務初始化失敗：資料庫遷移錯誤")?;

        let cache = GuildConfigCache::new(cache_capacity, cache_ttl_seconds);
        let transaction_manager =
            ConfigTransactionManager::new(repository.clone(), transaction_timeout);
    ...
    pub async fn get_config(&self, guild_id: i64) -> Result<Option<GuildConfig>> {
        ...
        let result = self
            ...
            .execute_read_transaction(vec![guild_id], |repo| {
                Box::pin(async move { repo.get_config(guild_id).await })
            })
            .await
            .context("獲取配置事務執行失敗")?;
    ...
    pub async fn update_config(&self, config: &GuildConfig) -> Result<ConfigUpdateResult> {
        ...
        let result = self
            ...
            .execute_write_transaction(vec![config.guild_id], move |repo| {
                Box::pin(async move { repo.upsert_config(&config_clone).await })
            })
            .await
            .context("更新配置事務執行失敗")?;
        ...
        match result {
            ...
            TransactionResult::Aborted(error) => {
                error!(
                    "配置更新事務失敗: guild_id={}, error={}",
                    config.guild_id, error
                );
    ...
    pub async fn delete_config(&self, guild_id: i64) -> Result<bool> {
        ...
        let result = self
            ...
            .execute_write_transaction(vec![guild_id], |repo| {
                Box::pin(async move { repo.delete_config(guild_id).await })
            })
            .await
            .context("刪除配置事務執行失敗")?;
    ...
    ///
    /// * `asset` - 要創建的背景圖片資源
    pub async fn create_background_asset(
        &self,
        ...
        let result = self
            ...
            .execute_write_transaction(vec![], move |repo| {
                Box::pin(async move { repo.create_background_asset(&asset_clone).await })
            })
            .await
    ...
    pub async fn get_background_asset(&self, asset_id: &str) -> Result<Option<BackgroundAsset>> {
        ...
        let result = self
            ...
            .execute_read_transaction(vec![], move |repo| {
                Box::pin(async move { repo.get_background_asset(&asset_id_str).await })
            })
            .await
            .context("獲取背景資源事務執行失敗")?;

        match result {
            TransactionResult::Committed(asset_opt) => Ok(asset_opt),
            TransactionResult::Aborted(error) => {
                error!(
                    "獲取背景圖片資源事務失敗: id={}, error={}",
                    asset_id_for_error, error
                );
...
mod tests {
    ...
    async fn test_config_lifecycle() {
        ...
        assert_eq!(
            retrieved_updated.unwrap().background_ref,
            Some("bg_test".to_string())
        );
    ...
    async fn test_background_asset_operations() {
        ...
        let create_result = service
            .create_background_asset(&asset)
            .await
            .expect("創建資源失敗");
        assert!(matches!(create_result, ConfigUpdateResult::Success));

        // 測試獲取背景資源
        let retrieved_asset = service
