...
mod tests {
    ...
    async fn test_config_crud_operations() {
        ...
        // 測試查詢存在的配置
        let retrieved_config = repo.get_config(guild_id).await.expect("查詢失敗")
            .expect("配置應該存在");
        ...
        repo.upsert_config(&updated_config).await.expect("更新配置失敗");
        
        let retrieved_updated = repo.get_config(guild_id).await.expect("查詢失敗")
            .expect("配置應該存在");
    ...
    async fn test_background_asset_operations() {
        ...
        repo.create_background_asset(&asset).await.expect("創建資源失敗");
        
        // 測試查詢背景圖片資源
        let retrieved_asset = repo.get_background_asset("test_asset").await.expect("查詢失敗")
            .expect("資源應該存在");
        ...
        // 測試刪除背景圖片資源
        let deleted = repo.delete_background_asset("test_asset").await.expect("刪除失敗");
        assert!(deleted, "刪除應該成功");
        
        let result = repo.get_background_asset("test_asset").await.expect("查詢失敗");
    ...
    async fn test_get_all_configs() {
        ...
        for config in &configs {
            let retrieved = all_configs.get(&config.guild_id)
                .expect("配置應該存在");
    ...
    async fn test_config_statistics() {
        ...
        let (total, with_channel, with_background) = repo.get_config_statistics()
            .await.expect("查詢統計失敗");
