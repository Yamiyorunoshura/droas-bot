...
impl GuildConfigCache {
    ...
    pub fn new(max_capacity: Option<u64>, ttl_seconds: Option<u64>) -> Self {
        ...
        info!("初始化公會配置緩存: capacity={}, ttl={}s", capacity, ttl.as_secs());
...
impl GuildConfigCache {
    ...
    /// * `guild_id` - 公會 ID
    /// * `last_update` - 最後更新時間戳
    pub async fn invalidate_if_stale(&self, guild_id: i64, last_update: chrono::DateTime<chrono::Utc>) {
    ...
    async fn test_warm_up() {
        ...
        let load_fn = |guild_id: i64| async move {
            if guild_id == 999 {
                ...
                Ok(Some(GuildConfig::new(guild_id, Some(guild_id + 1000), None)))
            }
        };
