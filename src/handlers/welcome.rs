...
impl WelcomeHandler {
    ...
    async fn process_welcome_message(&self, guild_id: u64, user_id: u64) -> DroasResult<()> {
        ...
        let welcome_channel_id = if let Some(guild_service) = &self.guild_service {
            let guild_config = self
                .get_guild_config(guild_service, &guild_id.to_string())
                .await?;
            if let Some(config) = guild_config {
                debug!(
                    "公會 {} 配置查詢結果: welcome_channel_id={}",
                    guild_id, config.welcome_channel_id
                );
                Some(
                    config
                        .welcome_channel_id
                        .parse::<u64>()
                        .map_err(|e| DroasError::validation(format!("無效的歡迎頻道 ID: {}", e)))?,
                )
            } else {
        ...
        // 4. 發送歡迎訊息到 Discord
        self.send_welcome_message(channel_id, user_id, &welcome_message)
            .await?;
    ...
    async fn get_guild_config(
        ...
        guild_service: &GuildConfigService,
        guild_id: &str,
    ) -> DroasResult<Option<GuildConfig>> {
