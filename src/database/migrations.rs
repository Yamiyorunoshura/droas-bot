...
pub async fn apply_migration(pool: &SqlitePool, migration: &Migration) -> Result<()> {
    info!("應用遷移: {} - {}", migration.version, migration.description);
...
pub async fn rollback_last_migration(pool: &SqlitePool) -> Result<Option<String>> {
    ...
    // 獲取最後應用的遷移
    let last_migration: Option<(String,)> = sqlx::query_as(
    ...
    if let Some((version,)) = last_migration {
        ...
        match version.as_str() {
            MIGRATION_VERSION_002 => {
                // 回滾索引創建
                sqlx::query("DROP INDEX IF EXISTS idx_guild_config_updated_at").execute(&mut *tx).await?;
                sqlx::query("DROP INDEX IF EXISTS idx_guild_config_background_ref").execute(&mut *tx).await?;
                sqlx::query("DROP INDEX IF EXISTS idx_background_asset_created_at").execute(&mut *tx).await?;
                sqlx::query("DROP INDEX IF EXISTS idx_background_asset_media_type").execute(&mut *tx).await?;
                sqlx::query("DROP INDEX IF EXISTS idx_background_asset_file_size").execute(&mut *tx).await?;
            },
            MIGRATION_VERSION_001 => {
                // 回滾表創建
                sqlx::query("DROP TABLE IF EXISTS background_asset").execute(&mut *tx).await?;
                sqlx::query("DROP TABLE IF EXISTS guild_config").execute(&mut *tx).await?;
            },
            _ => {
...
mod tests {
    ...
    async fn test_rollback_migration() {
        ...
        // 驗證該遷移確實被回滾
        let applied = is_migration_applied(pool, MIGRATION_VERSION_002).await.unwrap();
        assert!(!applied);
    }
