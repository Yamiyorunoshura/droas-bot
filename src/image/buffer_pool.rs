...
use crate::image::{WelcomeImageResult, WelcomeImageError, WELCOME_IMAGE_WIDTH, WELCOME_IMAGE_HEIGHT};
...
impl ImageBuffer {
    ...
    pub fn fill_rect(&mut self, x: u32, y: u32, width: u32, height: u32, color: [u8; 4]) -> WelcomeImageResult<()> {
    ...
    pub fn draw_circle(&mut self, center_x: u32, center_y: u32, radius: u32, color: [u8; 4], antialiasing: bool) -> WelcomeImageResult<()> {
    ...
    pub fn composite(&mut self, overlay: &ImageBuffer) -> WelcomeImageResult<()> {
        for y in 0..self.height().min(overlay.height()) {
            for x in 0..self.width().min(overlay.width()) {
                ...
                let blended = Rgba([
                    ((overlay_pixel.0[0] as f32 * alpha + base_pixel.0[0] as f32 * inv_alpha) as u8),
                    ((overlay_pixel.0[1] as f32 * alpha + base_pixel.0[1] as f32 * inv_alpha) as u8),
                    ((overlay_pixel.0[2] as f32 * alpha + base_pixel.0[2] as f32 * inv_alpha) as u8),
                    ((overlay_pixel.0[3] as f32 + base_pixel.0[3] as f32 * inv_alpha) as u8).min(255),
                ]);
...
impl BufferPool {
    ...
    pub async fn new(max_size: usize) -> WelcomeImageResult<Self> {
        ...
        for _ in 0..max_size.min(3) { // Start with a few buffers
