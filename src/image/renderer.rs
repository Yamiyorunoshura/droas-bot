...
use crate::image::{
    WelcomeImageResult, WelcomeImageError, BufferPool, ImageBuffer,
    ContrastCalculator, AvatarFetcher, AvatarProcessor, ProcessedAvatar,
    TextRenderer, TextStyle, WELCOME_IMAGE_WIDTH, WELCOME_IMAGE_HEIGHT,
};
...
impl ImageRenderer {
    ...
    pub async fn new(cache_dir: &Path) -> WelcomeImageResult<Self> {
        ...
        if !cache_dir.exists() {
            tokio::fs::create_dir_all(cache_dir).await
                .map_err(|e| WelcomeImageError::InvalidConfig(format!("Failed to create cache dir: {}", e)))?;
        }
    ...
    pub async fn render_welcome_image(&mut self, config: &WelcomeImageConfig) -> WelcomeImageResult<Vec<u8>> {
        ...
        let mut buffer = self.buffer_pool.get_buffer().await
            .map_err(|e| WelcomeImageError::BufferPool(format!("Failed to get buffer: {:?}", e)))?;
        
        // Step 1: Load and apply background
        self.apply_background(&mut buffer, &config.background_path).await
            .map_err(|e| WelcomeImageError::ImageProcessing(
        ...
        self.render_username(&mut buffer, &config.username).await
            .map_err(|e| WelcomeImageError::TextRendering(format!("Text rendering failed: {:?}", e)))?;
        ...
        self.buffer_pool.return_buffer(buffer).await
            .map_err(|e| WelcomeImageError::BufferPool(format!("Failed to return buffer: {:?}", e)))?;
    ...
    fn validate_config(&self, config: &WelcomeImageConfig) -> WelcomeImageResult<()> {
        ...
        if let Some(bg_path) = &config.background_path {
            if !bg_path.exists() {
                return Err(WelcomeImageError::InvalidConfig(
                    format!("Background file does not exist: {:?}", bg_path)
                ));
            }
        }
    ...
    async fn apply_background(&self, buffer: &mut ImageBuffer, background_path: &Option<PathBuf>) -> WelcomeImageResult<()> {
        match background_path {
            Some(path) if path.exists() => {
                // Load custom background
                let background_img = image::open(path)
    ...
    async fn render_avatar(&self, buffer: &mut ImageBuffer, avatar_url: &str) -> WelcomeImageResult<()> {
        // Fetch avatar from Discord CDN
        let avatar_image = self.avatar_fetcher.fetch_avatar(avatar_url).await
            .map_err(|e| WelcomeImageError::AvatarFetch(e))?;
            
        // Process avatar with circular mask
        let processed_avatar = self.avatar_processor.process_avatar(&avatar_image, Some(120))?;
    ...
    async fn render_username(&mut self, buffer: &mut ImageBuffer, username: &str) -> WelcomeImageResult<()> {
        ...
        let bg_luminance = self.contrast_calculator.calculate_average_luminance(
            buffer, sample_x, sample_y, sample_width, sample_height
        )?;
