...
impl ContrastCalculator {
    ...
    pub fn sample_background_luminance(
        &self,
        image_buffer: &crate::image::ImageBuffer,
        x: u32,
        y: u32,
        sample_size: u32,
    ) -> WelcomeImageResult<f64> {
    ...
    pub fn get_optimal_text_color(
        ...
        let avg_luminance =
            self.sample_background_luminance(image_buffer, center_x, center_y, sample_size)?;

        // Convert average luminance back to approximate RGB for contrast calculation
        let bg_approx = RgbColor {
            r: avg_luminance,
            g: avg_luminance,
            b: avg_luminance,
        };
