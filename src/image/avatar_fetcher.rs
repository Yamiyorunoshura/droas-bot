...
impl AvatarFetcher {
    ...
    pub async fn new() -> WelcomeImageResult<Self> {
        let client = Client::builder()
            ...
            .build()
            .map_err(|e| {
                AvatarFetchError::Network(format!("Failed to create HTTP client: {}", e))
            })?;
    ...
    pub async fn fetch_avatar(&self, avatar_url: &str) -> Result<DynamicImage, AvatarFetchError> {
        ...
        if !avatar_url.starts_with("https://") {
            return Err(AvatarFetchError::InvalidUrl(
        ...
        // Cache the image
        self.cache_avatar(avatar_url.to_string(), image.clone())
            .await;
