...
/// Global font cache
static FONT_CACHE: Lazy<Font<'static>> = Lazy::new(|| {
    Font::try_from_bytes(DEFAULT_FONT_DATA)
...
impl TextRenderer {
                ...
                if let Some(bounding_box) = glyph.pixel_bounding_box() {
                    glyph.draw(|x, y, v| {
                        ...
                        if px < target.width() && py < target.height() {
                            ...
                            if alpha > 0 {
                                self.blend_pixel(target, px, py, Rgba([
                                    outline_color[0],
                                    outline_color[1],
                                    outline_color[2],
                                    alpha
    ...
    fn blend_pixel(&self, target: &mut RgbaImage, x: u32, y: u32, pixel: Rgba<u8>) {
        ...
        if final_alpha > 0.0 {
            target_pixel[0] = ((pixel[0] as f32 * src_alpha + target_pixel[0] as f32 * dst_alpha * inv_src_alpha) / final_alpha) as u8;
            target_pixel[1] = ((pixel[1] as f32 * src_alpha + target_pixel[1] as f32 * dst_alpha * inv_src_alpha) / final_alpha) as u8;
            target_pixel[2] = ((pixel[2] as f32 * src_alpha + target_pixel[2] as f32 * dst_alpha * inv_src_alpha) / final_alpha) as u8;
